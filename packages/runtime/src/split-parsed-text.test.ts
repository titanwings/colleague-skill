...
describe("parsed text splitting", () => {
  it(
    "keeps every part within the limit and loses nothing for a size sweep",
    { timeout: 30_000 },
    () => {
      const line = `${"a".repeat(63)}\n`;
      // Sizes stay at or below two materials: the sweep has to finish inside a test timeout even
      // on a slow shared CI runner, and two sizes already cover both sides of the cut.
      for (const size of [maximumBytes - 1, maximumBytes, maximumBytes + 1, 2 * maximumBytes]) {
  ...
  it(
    "does not emit an empty trailing part for an exact multiple of the limit",
    { timeout: 30_000 },
    () => {
  ...
  it("never cuts between a high and a low surrogate", { timeout: 30_000 }, () => {
  ...
  it(
    "terminates when a combining mark cannot be kept with the code point before it",
    { timeout: 30_000 },
    () => {
  ...
  it("cuts a single over-long line in linear time", { timeout: 30_000 }, () => {
    const timings: number[] = [];
    for (const mebibytes of [1, 2, 4]) {
    ...
    const [one, two, four] = timings as [number, number, number];
    expect(two).toBeLessThan(Math.max(1_000, one * 6 + 300));
    expect(four).toBeLessThan(Math.max(1_500, one * 12 + 500));
  });

  it(
    "does not start a part with a combining mark even at a line boundary",
    { timeout: 30_000 },
    () => {
  ...
  it("does not end a part with spaces or tabs the engine would strip", { timeout: 30_000 }, () => {
  ...
  it(
    "canonicalizes before splitting, so CRLF and NFC-expanding text stay legal",
    { timeout: 30_000 },
    () => {
  ...
  it("terminates and stays byte-exact on long runs of spaces or tabs", { timeout: 30_000 }, () => {
  ...
  it(
    "refuses a whitespace run no legal part could hold instead of failing the call",
    { timeout: 30_000 },
    () => {
      for (const text of [
  ...
  it("never leaves a part that is only whitespace", { timeout: 30_000 }, () => {
  ...
  it(
    "refuses a whitespace run as long as one material instead of emitting a blank part",
    { timeout: 30_000 },
    () => {
      ...
      expect(() => splitParsedText("a\u00a0\u00a0", 4)).toThrowError(
        /whitespace at least as long/u,
      );
  ...
  it("is deterministic for the same input", { timeout: 30_000 }, () => {
    const text = `${"😀 line\n".repeat(90_000)}tail`;
