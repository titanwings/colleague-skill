...
| 5 | `ds/05-agents` | coding-agent 矩阵（命令/目录/能力/双语注意）+ 双语文档 + 断言测试 | 1 | `node --test tests/agents.test.mjs` |
...
| — | `dot-skill-test`（本分支） | 契约、验收协议、语料夹具、验收脚本、本状态表 | — | `node scripts/acceptance.mjs`（依赖到位后必须全绿） |
